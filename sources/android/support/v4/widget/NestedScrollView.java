...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    public boolean executeKeyEvent(KeyEvent keyEvent) {
        ...
        int i = TapdaqError.NETWORKS_FAILED_TO_LOAD_AD;
        if (!canScroll) {
            ...
            View findNextFocus = FocusFinder.getInstance().findNextFocus(this, findFocus, TapdaqError.NETWORKS_FAILED_TO_LOAD_AD);
            if (findNextFocus == null || findNextFocus == this || !findNextFocus.requestFocus(TapdaqError.NETWORKS_FAILED_TO_LOAD_AD)) {
                return false;
            }
            return true;
            ...
            if (keyCode != 62) {
                switch (keyCode) {
                    ...
                    case 20:
                        if (!keyEvent.isAltPressed()) {
                            return arrowScroll(TapdaqError.NETWORKS_FAILED_TO_LOAD_AD);
                        }
                        return fullScroll(TapdaqError.NETWORKS_FAILED_TO_LOAD_AD);
                    default:
                        return false;
                }
            } else {
    ...
    public boolean onRequestFocusInDescendants(int i, Rect rect) {
        View view;
        if (i == 2) {
            i = TapdaqError.NETWORKS_FAILED_TO_LOAD_AD;
        } else if (i == 1) {
            i = 33;
        }
        if (rect == null) {
