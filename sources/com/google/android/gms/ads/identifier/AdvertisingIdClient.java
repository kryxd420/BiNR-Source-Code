...
public class AdvertisingIdClient {
    ...
    private final boolean zza(Info info, boolean z, float f, long j, String str, Throwable th) {
        ...
        hashMap.put("app_context", z ? TapjoyAuctionFlags.AUCTION_TYPE_FIRST_PRICE : "0");
        if (info != null) {
            hashMap.put("limit_ad_tracking", info.isLimitAdTrackingEnabled() ? TapjoyAuctionFlags.AUCTION_TYPE_FIRST_PRICE : "0");
        }
        if (!(info == null || info.getId() == null)) {
            hashMap.put("ad_id_size", Integer.toString(info.getId().length()));
        }
        if (th != null) {
            hashMap.put(TJAdUnitConstants.String.VIDEO_ERROR, th.getClass().getName());
        }
